...
document.addEventListener('DOMContentLoaded', () => {                               // DOM이 준비되면 실행
  ...
  if (!detail) return;                                                              // 상세 영역이 없으면 종료
  ...
  // ---------- 상세 값 채우기 ----------
  function fillItemDetail(slide, data) {                                            // 상세 슬라이드에 데이터 주입
    data = data || {};
  ...
  function enterEdit() {                                                                                   // 수정 모드로 전환
    ...
    setHTML(detail.querySelector('#d-itemName'),
      `<input type="text" id="e-itemName" value="${state.backup.itemName}" autocomplete="off">`);          // 이름 입력
    detail.querySelector('#e-itemName')?.focus();                                                          // 포커스
    ...
    setHTML(detail.querySelector('#d-clientName'),
      `<input type="text" id="e-vendorName" value="${state.backup.clientName}" placeholder="거래처명(전송안함)">`); // 거래처명 입력

    setHTML(detail.querySelector('#d-itemDiv'),
      `<input type="text" id="e-itemDiv" value="${state.backup.itemDiv}">`);                               // 구분 입력
    ...
    setHTML(detail.querySelector('#d-itemUnit'),
      `<input type="text" id="e-unit" value="${state.backup.itemUnit}">`);                                 // 단위 입력
  ...
  function exitEdit(restore) {                                                                             // 수정 종료
    ...
    if (restore) {                                                                                         // 복구 모드면 백업 복원
      ...
      const vName  = detail.querySelector('#e-itemName')?.value || '';
      const vCid   = detail.querySelector('#e-vendorId')?.value || '';
      const vCnm   = detail.querySelector('#e-vendorName')?.value || '';
      const vDiv   = detail.querySelector('#e-itemDiv')?.value || '';
      const vPrice = detail.querySelector('#e-unitPrice')?.value || '';
      const vUnit  = detail.querySelector('#e-unit')?.value || '';

      setHTML(detail.querySelector('#d-itemName'), vName);
      setHTML(detail.querySelector('#d-clientId'), vCid);
      setHTML(detail.querySelector('#d-clientName'), vCnm);
      setHTML(detail.querySelector('#d-itemDiv'), vDiv);
      setHTML(detail.querySelector('#d-itemPrice'), (vPrice ? Number(vPrice).toLocaleString() : ''));
      setHTML(detail.querySelector('#d-itemUnit'), vUnit);
    }
  ...
  async function saveEdit() {                                                                               // 저장 실행
    ...
    const itemName  = detail.querySelector('#e-itemName')?.value?.trim() || '';                            // 필수
    const itemDiv   = detail.querySelector('#e-itemDiv')?.value?.trim() || '';                             // 구분
    const unitPrice = detail.querySelector('#e-unitPrice')?.value?.trim() || '';                           // 단가
    const unit      = detail.querySelector('#e-unit')?.value?.trim() || '';                                // 단위
    ...
    try {
      ...
      const saved = raw ? JSON.parse(raw) : { item_id: itemId, item_name: itemName, item_div: itemDiv, item_price: Number(unitPrice||0), item_unit: unit };
