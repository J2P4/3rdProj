...
function bindworker() {
  ...
  const chkAll      = document.getElementById('chkAll');          // 마스터 체크(목록 헤더)
  ...
  // === 상세 열기: 해당 목록 테이블의 tbody를 스코프로 사용 ===
  // chkAll이 속한 table 기준으로 tbody를 특정 (다른 테이블과 간섭 방지)
  const table = chkAll ? chkAll.closest('table') : document.querySelector('.table table');
  const tbody = table?.tBodies[0];

  if (detailSlide && tbody) {
    // 행 클릭 → 상세 (체크박스/첫 번째 칸/버튼/링크 등 제외)
    tbody.addEventListener('click', async (e) => {
      const td = e.target.closest('td');
      if (!td) return;
      if (td.cellIndex === 0) return; // 체크박스 칸 무시
      ...
      const workerId = tr?.getAttribute('data-id');
      if (!workerId) return;
      ...
      try {
        ...
        const wid = data?.worker_id || workerId;
  ...
  // === 마스터 ↔ 개별 체크 동기화 (같은 tbody 기준) ===
  if (chkAll && tbody) {
    const getItems = () => Array.from(tbody.querySelectorAll('input.rowChk:not([disabled])'));

    // 마스터 → 전체
    chkAll.addEventListener('change', () => {
      getItems().forEach(chk => { chk.checked = chkAll.checked; });
    });

    // 개별 → 마스터 (tbody에 바인딩: 다른 전파차단 코드와 충돌 없음)
    tbody.addEventListener('change', (e) => {
      const t = e.target;
      if (!(t instanceof HTMLInputElement) || !t.classList.contains('rowChk')) return;
